...
router.get('/',async(req,res)=>{
  const date=req.query.date||new Date().toISOString().slice(0,10);const q=String(req.query.q||'').trim();
  ...
  const params=[date];if(q){const like=`%${q}%`;sql+=` AND (ts.title LIKE ? OR c.name LIKE ? OR c.customer_code LIKE ? OR COALESCE(e.name,u.name) LIKE ? OR t.ticket_code LIKE ? OR s.code LIKE ? OR cl.name LIKE ?)`;params.push(like,like,like,like,like,like,like);}sql+=` ORDER BY ts.schedule_time,COALESCE(e.name,u.name)`;
  ...
  res.render('schedules/index',{title:'Jadwal Teknisi',date,q,schedules,staff,customers,tickets,sites});
});
