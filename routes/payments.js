...
async function postCashTransaction(conn,{paymentId,invoiceId,amount,reference,categoryName='Pendapatan Billing',prefix='Pembayaran',actorUserId=null}){
  ...
  await conn.execute(`INSERT INTO cash_transactions(transaction_date,name,category_id,site_id,amount,notes,source_type,source_id,created_by) VALUES(CURDATE(),?,?,?,?,?,'payment',?,?)`,[
    `${prefix} ${meta.customer_name}`,catId,meta.site_id,amount,`Invoice ${meta.invoice_number}${reference?` · ${reference}`:''}`,paymentId,actorUserId
  ]);
}
...
router.get('/',async(req,res)=>{
  const [payments]=await db.query(`SELECT p.*,i.invoice_number,c.customer_code,c.name customer_name,s.code site_code,u.name collector_name,v.name verifier_name,pu.name proof_uploader_name
  ...
  const [openInvoices]=await db.query(`SELECT i.id,i.invoice_number,i.outstanding,c.customer_code,c.name customer_name,s.code site_code FROM invoices i JOIN customers c ON c.id=i.customer_id JOIN sites s ON s.id=c.site_id WHERE i.status IN ('unpaid','partial','overdue') AND i.outstanding>0 ORDER BY i.due_date`);
  ...
  res.render('payments/index',{title:'Pembayaran',payments,openInvoices,staff,summary:summary||{}});
});

router.post('/',async(req,res)=>{
  const {invoice_id,amount,method,reference,notes,collector_user_id,bank_name,payment_status}=req.body;
  const normalizedMethod=method||'transfer';
  const numericAmount=Number(amount);
  if(!Number.isFinite(numericAmount) || numericAmount<=0) throw new Error('Nominal pembayaran harus lebih dari 0.');
  if(normalizedMethod==='transfer' && !req.file) throw new Error('Bukti transfer wajib diupload untuk pembayaran transfer.');
  const isAdmin=req.session.user.role==='admin';
  // Transfer recorded by staff always enters verification queue. Admin may verify immediately when explicitly selected.
  const status=normalizedMethod==='cash'?'confirmed':(normalizedMethod==='transfer' ? (isAdmin && payment_status==='confirmed'?'confirmed':'pending') : (payment_status==='pending'?'pending':'confirmed'));
  const settlement=normalizedMethod==='cash'?'held_by_staff':'not_applicable';
  // Non-admin staff can only record cash under their own name.
  ...
  try{
    ...
    const [invoiceRows]=await conn.execute(`SELECT id,outstanding,status FROM invoices WHERE id=? FOR UPDATE`,[invoice_id]);
    if(!invoiceRows.length) throw new Error('Invoice tidak ditemukan.');
    if(['paid','cancelled','refunded'].includes(invoiceRows[0].status) || Number(invoiceRows[0].outstanding)<=0) throw new Error('Invoice sudah tidak memiliki tagihan yang perlu dibayar.');
    if(numericAmount>Number(invoiceRows[0].outstanding)) throw new Error(`Nominal melebihi sisa tagihan (${Number(invoiceRows[0].outstanding).toLocaleString('id-ID')}).`);
    if(req.file) savedProof=await saveProofFile(req.file);
    const [r]=await conn.execute(`INSERT INTO payments (invoice_id,amount,method,reference,notes,status,settlement_status,bank_name,proof_reference,proof_path,proof_original_name,proof_mime,proof_size,proof_uploaded_by,proof_uploaded_at,paid_at,received_by,collector_user_id,verified_by,verified_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,?,?,?)`,[
      invoice_id,numericAmount,normalizedMethod,reference||null,notes||null,status,settlement,bank_name||null,savedProof?.originalName||null,savedProof?.filename||null,savedProof?.originalName||null,savedProof?.mime||null,savedProof?.size||null,savedProof?req.session.user.id:null,savedProof?new Date():null,req.session.user.id,collector,status==='confirmed'?req.session.user.id:null,status==='confirmed'?new Date():null
    ]);
    paymentId=r.insertId;
    await refreshInvoiceStatus(conn,invoice_id);
    if(status==='confirmed' && normalizedMethod!=='cash') await postCashTransaction(conn,{paymentId,invoiceId:invoice_id,amount:numericAmount,reference,actorUserId:req.session.user.id});
    await conn.commit();
    await audit({userId:req.session.user.id,action:'create',entityType:'payment',entityId:paymentId,description:`Pembayaran ${normalizedMethod} invoice ${invoice_id} Rp${numericAmount}${savedProof?' · bukti terupload':''}${normalizedMethod==='cash'?' · cash dipegang staff':''}`,ip:req.ip});
    if(status==='confirmed') await maybeAutoUnisolate(invoice_id);
    req.session.flash={type:'success',message:normalizedMethod==='cash'?'Pembayaran cash tercatat. Invoice diperbarui, uang masuk status Cash di Staff sampai disetor.':status==='pending'?'Transfer dan bukti pembayaran tersimpan. Menunggu verifikasi admin.':'Pembayaran dan bukti transfer berhasil disimpan.'};
  }catch(e){await conn.rollback();if(savedProof) await removeProofFile(savedProof.filename);throw e;}finally{conn.release();}
...
router.get('/:id/proof',async(req,res)=>{
  ...
  const p=rows[0];
  if(!p?.proof_path) return res.status(404).send('Bukti pembayaran tidak ditemukan.');
...
router.post('/:id/proof',requireAdmin,async(req,res)=>{
...
router.post('/:id/verify',requireAdmin,async(req,res)=>{
  ...
  try{
    ...
    if(p.status!=='confirmed'){
      ...
      if(!invoiceRows.length) throw new Error('Invoice pembayaran tidak ditemukan.');
    ...
    req.session.flash={type:'success',message:'Pembayaran diverifikasi berdasarkan bukti transfer dan invoice diperbarui.'};
  }catch(e){await conn.rollback();throw e;}finally{conn.release();}
  res.redirect('/payments');
});

router.get('/reconciliation',requireAdmin,async(req,res)=>{
  const [held]=await db.query(`SELECT p.*,c.customer_code,c.name customer_name,s.code site_code,u.name collector_name,i.invoice_number
    ...
    WHERE p.method='cash' AND p.status='confirmed' AND p.settlement_status='held_by_staff' ORDER BY u.name,p.paid_at`);
  ...
  const [[summary]]=await db.query(`SELECT
    ...
    FROM payments`);
  res.render('payments/reconciliation',{title:'Rekonsiliasi Pembayaran',held,staffBalances,summary:summary||{}});
});
